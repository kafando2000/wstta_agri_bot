...
#include <random>
#include "custom_interfaces/action/reach_location.hpp"
#include "rclcpp/rclcpp.hpp"
...
public:
    using ReachLocation = custom_interfaces::action::ReachLocation;
    using GoalHandleReachLocation = rclcpp_action::ServerGoalHandle<ReachLocation>;

    explicit ReachLocationActionServer() : Node("reach_location_action_server") {
...
private:
    ...
    rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID & uuid,std::shared_ptr<const ReachLocation::Goal> goal) {

        RCLCPP_INFO(this->get_logger(), "Received goal request for location (%f, %f) with timeout %f", goal->x, goal->y, goal->timeout);
        (void)uuid;
    ...
    void execute(const std::shared_ptr<GoalHandleReachLocation> goal_handle) {
        ...
        while ((this->now() - start_time).seconds() < timeout) {
            ...
            current_x += k*(target_x-current_x)+kd*velocity;  // Simplified movement model, assumes movement in x direction only
            current_y += k*(target_y-current_y)+kd*velocity;  // Simplified movement model, assumes movement in y direction only
