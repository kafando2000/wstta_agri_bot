...
using Action = custom_interfaces::action::ReachLocation;
...
class WaitForServer : public BT::StatefulActionNode {
  public:
    WaitForServer(const std::string & action_name, const BT::NodeConfig & conf) : BT::StatefulActionNode(action_name, conf),
        _node{conf.blackboard->rootBlackboard()->get<rclcpp::Node::SharedPtr>("node")} 
    {    
         std::cout << "creating second node"<<std::endl;
    }
