...
public:
  ...
  ActionServer() : Node("action_server") {
    ...
    // create action server with action type and binding
...
private:
  ...
  // handle the goal
  rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID &uuid,std::shared_ptr<const MyCustomAction::Goal> goal) {
    (void)uuid;
